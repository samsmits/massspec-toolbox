...
filename_script = 'run-myrimatch.xinteract-combined.sh';
f_script = open(filename_script,'w')
f_script.write('#!/bin/bash\n')
f_script.write('SAMPLE_NAME="tmp"\n')

for filename_pepxml in os.listdir('myrimatch'):
    filename_pepxml = filename_pepxml.strip()
    if( not filename_pepxml.endswith('-myrimatch.pepxml') ):
        continue
    
    filename_base = filename_pepxml.replace('.pepxml','').replace('-myrimatch','.myrimatch')
    filename_pepxml = os.path.join('myrimatch',filename_pepxml)

    f_script.write('cp %s %s\n'%(filename_pepxml,'tmp/'))

filename_base = os.path.join('myrimatch.xinteract','$SAMPLE_NAME')
...
filename_prot_nonsp = filename_base+'.xinteract_NONSP.prot.xml'
