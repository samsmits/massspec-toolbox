...
f_out.write("#Spectrum_id\tCharge\tNeutralMass\tPeptide\tProtein\tMissedCleavages\tAbsScore(Xcorr)\tRelScore(DeltaCn)\n")
for spectrum_id in PSM.keys():
    ...
    f_out.write("%s\t%s\t%f\t%s\t%s\t%d\t%f\t%f\n"%(spectrum_id,charge,neutral_mass,best_peptide,best_protein,missed_cleavages,best_xcorr,best_deltacn))
f_out.close()
