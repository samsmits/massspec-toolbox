...
f_out.write("#Spectrum_id\tCharge\tNeutralMass\tPeptide\tProtein\tAbsScore(Hyperscore)\tRelScore(Expect)\n")
for spectrum_id in PSM.keys():
    ...
    f_out.write("%s\t%s\t%f\t%s\t%s\t%f\t%f\n"%(spectrum_id,charge,neutral_mass,best_peptide,best_protein,best_hyperscore,best_expect))
f_out.close()
